...
The input side of a fan-out: chunk a large parsed set (CSV/XLSX rows) so each
chunk becomes its own downstream job with a bounded, serialisable payload,
rather than one job carrying everything. "Split a CSV into jobs of 500" is then
``batched(csv.read_rows(data), 500)``.
"""
