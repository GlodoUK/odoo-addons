...
class EdiSaleOrder(models.Model):
    ...
    odoo_id = fields.Many2one("sale.order", string="Sale Order", required=True)

    edi_message_id = fields.Many2one(
        "edi.message", index=True, required=True, auto_join=True,
    )

    edi_external_id = fields.Char(string="EDI External Ref")

    edi_order_line_ids = fields.One2many("edi.sale.order.line", "edi_order_id",)
    ...
    edi_metadata_string = fields.Char(
        compute="_compute_edi_metadata_string", string="Metadata",
    )
