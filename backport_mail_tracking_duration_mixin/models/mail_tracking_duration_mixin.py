...
class MailTrackingDurationMixin(models.AbstractModel):
    ...
    _description = "Mixin to compute the time a record has spent in each value a"
    " many2one field can take"
    ...
    def _compute_duration_tracking(self):
        """
        Computes duration_tracking, a Json field stored as {
            <many2one_id (str)>: <duration_spent_in_seconds (int)>
        }

        e.g. {"1": 1230, "2": 2220, "5": 14}

        `_track_duration_field` must be present in the model that uses the mixin
        to specify on what field to compute time spent. Besides, tracking must be
        ...
        if (
            ...
            raise ValueError(
                _(
                    "Field “%(field)s” on model “%(model)s” must be of type Many2one"
    ...
    def _get_duration_from_tracking(self, trackings):
        """
        Calculates the duration spent in each value based on the provided list of
        trackings.

        It adds a "fake" tracking at the end of the trackings list to account for the
        time spent in the current value.
        ...
        Returns:
            dict: A dictionary where the keys are the IDs of the values, and the values
