"""Filesystem building blocks for pipeline stages.

Thin helpers over an fsspec filesystem for the two file operations pipelines
...
with no Odoo env. ``providers`` is the one exception -- it reads fsspec's
...
def archive(fs, src, directory):
    ...
    the scanned folder in one step, so a slow or failing stage can never leave
...
# transport (webdav, oci, ...) shows up in providers() with no code change here.
