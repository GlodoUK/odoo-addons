...
from odoo.addons.pipeline.tools import files
...
class TestFileTools(TransactionCase):
    """The fs tools are pure fsspec drivers, so they run against a real
    LocalFileSystem on a throwaway directory -- no Odoo env involved."""

    def setUp(self):
        super().setUp()
        self.fs = LocalFileSystem(auto_mkdir=True)
        self.root = tempfile.mkdtemp(prefix="pipeline-files-")
