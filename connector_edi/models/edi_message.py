...
class EdiMessage(models.Model):
    ...
    envelope_id = fields.Many2one(
        "edi.envelope", required=False, index=True, ondelete="cascade",
    )
    ...
    metadata_string = fields.Char(
        compute="_compute_metadata_string", string="Metadata",
    )
    ...
    model = fields.Char(
        index=True, required=False, readonly=True, string="Related Document",
    )
    ...
    def action_send(self):
        for record in self:
            ...
            self.env["edi.envelope"].with_delay()._enclose_messages(
                record.envelope_route_id, record,
            )
    ...
    def action_error(self, msg=None):
        for record in self:
            ...
            record.message_post(body=str(msg), subtype="connector_edi.mt_message_error")
