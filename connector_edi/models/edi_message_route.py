...
class EdiMessageRoute(models.Model):
    ...
    action = fields.Selection(
        [("none", "None"), ("code", "Python Code"), ("qweb", "Qweb Template")],
        required=True,
    ...
    queue_eta = fields.Integer(default=0)

    qweb_arch = fields.Text()
...
"""
    ...
    @api.constrains("action", "direction")
    def _check_qweb_direction_out_only(self):
        for record in self.filtered(lambda x: x.action == "qweb"):
            if record.direction != "out":
                raise ValidationError(
                    _("Qweb message actions are only supported on direction = out")
                )
