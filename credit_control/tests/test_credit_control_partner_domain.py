...
class TestCreditControlPartnerDomain(CreditControlTestCase):
    def test_partner_policy_matches(self):
        credit_control_policy_id = self.env["credit.control.policy"].create(
            {
                "name": "Partner Domain Test Policy",
                "rule_ids": [
                    (
                        0,
                        0,
                        {
                            "name": "partner_domain",
                            "rule": "partner_domain",
                            "partner_domain": [("id", "=", self.partner_id.id)],
    ...
    def test_partner_policy_not_matches(self):
        credit_control_policy_id = self.env["credit.control.policy"].create(
            {
                "name": "Partner Domain Test Policy",
                "rule_ids": [
